...
export interface SplitWithMeta {
  ...
  created_by: string
  /** Who paid the original expense (transaction creator). */
...
/**
 * Net position per member from unsettled shares.
 * Payer is owed each unsettled share; debtor owes their share_minor.
 */
...
/** Open debts as from→to pairs with share ids for marking settled. */
...
export function sharesSumValid(shareMinors: number[], amountMinor: number): boolean {
  const sum = shareMinors.reduce((a, b) => a + b, 0)
