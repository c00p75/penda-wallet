...
import type { SuggestedMission } from '@/src/api/missions';
import { addLocalDays, localDateStr, localMonthPrefix } from '@/src/lib/dates';
...
export function suggestMissions(transactions: Transaction[], now: Date = new Date()): SuggestedMission[] {
  ...
  const end5 = addLocalDays(now, 4);
  const end7 = addLocalDays(now, 6);
