...
export function InstallBanner() {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border bg-card p-4 shadow-xs">
      <button
        type="button"
        onClick={dismiss}
        aria-label="Dismiss install banner"
        className="absolute right-2 top-2 flex size-7 items-center justify-center rounded-full text-muted-foreground hover:bg-accent"
      >
        <X className="size-4" />
      </button>
      ...
      {canPrompt ? (
        ...
        <div className="mt-3 flex flex-col gap-1 rounded-xl bg-muted p-3 text-sm text-muted-foreground">
