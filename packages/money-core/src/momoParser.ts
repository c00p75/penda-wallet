...
export interface ParsedMoMo {
  ...
  /** ISO date (YYYY-MM-DD). Falls back to today when the text has no date. */
  transactionDate: string
}

// A currency-prefixed amount, e.g. "K1,250.00", "ZMW 450", "$20.50".
// The lookbehind stops "K" from matching mid-word.
const AMOUNT_RE = /(?<![A-Za-z])(K|ZMW|USD|\$)\s?([\d,]+(?:\.\d{1,2})?)/gi

const EXPENSE_RE =
  /\b(sent|paid|payment|debited|purchase|withdraw|withdrawn|withdrew|bought|deducted)\b/i
