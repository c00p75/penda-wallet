-- Let a single budget/envelope run over an arbitrary custom date range,
-- alongside the existing weekly/monthly cadences (a trip, a pay period, a
-- project). The top-level monthly spending_plans hero limit is untouched.
...
-- A custom range must have a real end on/after its start. Weekly/monthly
-- rows are unaffected, end_date stays null for them, as today.
alter table budgets add constraint budgets_custom_range_check
  check (period <> 'custom' or (end_date is not null and end_date >= start_date));

-- Rollover has no "next period" to carry into on a one-off range. Rejected
-- at the data layer as defense in depth alongside the UI hiding the toggle
-- and the RPC's carry loop no-op below.
alter table budgets add constraint budgets_custom_no_rollover_check
  check (period <> 'custom' or rollover = false);

-- Same 10-column return shape as 0055, so CREATE OR REPLACE is legal here.
...
begin
  ...
  for b in
    select id, category_id, amount_minor, period, rollover, start_date, end_date
    from budgets
